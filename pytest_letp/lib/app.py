"""Legato application library.
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Module related to the Legato application (build, install, verification, ...)

.. note::
    Use the legato fixture instead of calling directly the library.
...
def make(target_type, app_name, app_path="", should_fail=False, option="", timeout=600):
    ...
    :returns: make stdout if success

    :raises: AssertionError: If make not accorded to should_fail param
    """
    _service_names = [
        "audio",
        "cellNetService",
...
def install(
    ...
    :raises: AssertionError: if installation error
    """
...
def make_install_sys(
    ...
    Args:
        target_type: type of target (wp85, ar759x, ...)
        target_ip: ip address of the target
        sys_name: name of the system definition file
        sys_path: path of the system definition file
        quiet: if True, do not produce logs
        timeout: in seconds
...
def make_install(target_type, target_ip, app_name, app_path="", option=""):
    ...
    Args:
        ...
        option: extra mkapp options

    Returns:
        None

    Raises:
        AssertionError: if error
    """
...
def clean(target, app_name, remove_file=True, remove_app=True):
    ...
    Args:
        target: fixture to communicate with the target.
        app_name: name of the application.
        remove_file: clean the build directory or not.
        remove_app: remove the application or not.
...
def update_legato_cwe(target, file_path, timeout=120):
    ...
    Args:
        target: fixture to communicate with the target
        file_path: path of the file to install
        timeout: in second
...
class LegatoManager:
    ...
    def make(self, app_name, app_path="", should_fail=False, option="", timeout=600):
        ...
        Args:
            app_name: name of the application
            app_path: path of the application
            should_fail: make is expected to fail
            option: extra mkapp options
            timeout: timeout

        Returns:
            make stdout if success

        Raises:
            AssertionError: If make not accorded to should_fail param
        """
        return make(
    ...
    def install(self, app_name, app_path="", signed=False, should_fail=False):
        ...
        Args:
            app_name: name of the application
            signed: set to 1 if installing signed app, 0 otherwise
            app_path: path of the application
            timeout: allowed time to install the app

        Returns:
            None

        Raises:
            AssertionError: if installation error
        """
        install(
    ...
    def are_apps_running(self, app_names):
        ...
        Args:
            app_name: Application names on legato
    ...
    def clean(self, app_name, remove_file=True, remove_app=True):
        ...
        Args:
            app_name: name of the application
            remove_only: only remove the application. Do not clean the build directory
    ...
    def make_sys(
        ...
        Args:
            sys_name: name of the system definition file
            sys_path: path of the  system definition file
            append_to_version: string to append to version
            should_fail: make is expected to fail
            option: extra mksys options
            quiet: if True, do not produce logs
