...
class SwiModule:
    ...
    def sim_status(self, timeout=20):
        ...
        if not self.links[1].info.is_used() or self.__class__.__name__ == "HL79XX":
            ...
            status = re.search(r"\+KSREP:\s*[0|1]{1},(?P<status_id>\d{1})", rsp)
            # Skip checking mobile start-up reporting. LE-16756
            return status.group("status_id") if status is not None else "0"
