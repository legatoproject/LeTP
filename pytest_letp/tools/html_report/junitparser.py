...
class Attr(object):
    ...
    def __get__(self, instance, cls):
        """Gets value from attribute, return None if attribute doesn't
        exist."""
...
class XFail(Result):
    ...
    def __eq__(self, other):
        return super(XFail, self).__eq__(other)
...
class XPass(Result):
    ...
    def __eq__(self, other):
        return super(XPass, self).__eq__(other)
...
class Running(Result):
    ...
    def __eq__(self, other):
        return super(Running, self).__eq__(other)
...
class TestCase(Element):
    ...
    classname = Attr()
    time = FloatAttr()
    _possible_results = {Failure, Error, Skipped, XPass, XFail, Running}
    ...
    def result(self):
        ...
        if len(results) == 0:
            return None
        if len(results) > 1:
            print("Warning: %s has %d results" % (self.name, len(results)))
        return results[0]
